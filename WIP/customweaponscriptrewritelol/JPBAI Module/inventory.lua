...
function inventory.hasItemWithParameter(args)
  if args.parameters then
    local searchType = args.item or false
    local query = args.parameters or {}
    if self.findItemWithParam(query, args.greater) then return true end
  end
  return false
end

function inventory.findItemWithParam(parameter, searchType, returnGreaterIfAvailable)
  local currentItem = copy(itemDescriptor)
  if not searchType then
    player.getItemWithParameter()
  end
end