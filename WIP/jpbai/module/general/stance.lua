...
function setStance(stanceName) -- replace and expend on the old version in stances.lua
    ...
    if self.stances[stanceName] then
        ...
        sb.logError("[setStance] stance %s couldn't be found", stanceName)
    end
            ...
            for group, transform in pairs(self.stance.transformations or {}) do
                ...
                if transform.inherit then
                    ...
                    if inheritedValue.velocity then
                        velocity.translate = vec2.add(inheritedValue.velocity.translate or {0, 0}, translate or {0, 0})
                        velocity.rotate = (inheritedValue.velocity.rotate or 0) + (velocity.rotate or 0)
                        velocity.scale = (inheritedValue.velocity.scale or 0) + (velocity.scale or 0)
                    end
                end
                if velocity then
                    local rotationCenter = transform.rotationCenter or {0, 0}
